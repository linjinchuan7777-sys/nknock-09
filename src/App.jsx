...
function App() {
  ...
  const handleTabChange = (parentTab, subTab = null) => {
    ...
    if (parentTab === 'all') setCurrentView('home-view');
      ...
      if (subTab === '日記') setCurrentView('notes-view');
  ...
  const handleAddFriend = () => {
    ...
    // Check if friend exists in global pool (or mockup search)
    alert("已成功向該好友發送拜訪申請並自動加入好友活頁夾！");
    // Mock add a default friend from global pool or create one
    const newFriend = {
      ...
      favorites: { country: "英國", color: "綠色", music: "搖滾樂", movie: "動作片", food: "披薩", trait: "幽默" },
      top3: { title: "喜歡的影集", top1: "怪奇物語", top2: "絕命毒師", top3: "黑鏡" },
      social: { role: "好友夥伴", talent: "吉他 / 運動", welcome: "嗨，很高興認識你！", status: "今天天氣真好 ☀️" },
      aboutMe: { passion: 90, humor: 80, procrastination: 50, fitness: 75, foodie: 70 }
    };
    ...
    <div className="app-container">
      ...
      {modalType === 'profile' && (
        <ProfileModal 
          ...
          onClose={() => setModalType(null)}
          onSubmit={(newProfile) => {
            setOwnerProfile({ ...ownerProfile, ...newProfile });
            setModalType(null);
            alert("個人檔案儲存成功！");
          }}
        />
...
const PostModal = ({ post, onClose, onSubmit }) => {
  ...
  const [subcategory, setSubcategory] = useState(post ? post.subcategory : '情感');
    ...
    <div className="modal-overlay active" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="modal-content" style={{ maxWidth: '500px', width: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px' }}>
          ...
          <div className="form-group" style={{ textAlign: 'left' }}>
            <label className="form-label" style={{ fontWeight: 'bold', fontSize: '12px' }}>類別</label>
          ...
          <div className="form-group" style={{ textAlign: 'left' }}>
            ...
            <input type="text" className="form-input" value={subcategory} onChange={(e) => setSubcategory(e.target.value)} />
          </div>
